...
const maleFemale = str => {
  ...
  switch (str) {
    case "male":
      urlImg = '<img src="./hw12_ajax_img/icon/male.png" alt="Sex Icon"></img>';
      break;

    case "female":
      urlImg = '<img src="./hw12_ajax_img/icon/female.png" alt="Sex Icon"></img>';
      break;

    default:
      urlImg = '<img src="./hw12_ajax_img/icon/NA.png" alt="Sex Icon"></img>';
  }
...
function makeCharacters() {
  ...
  return axios(config).then(res => res.data.characters).then(hero => hero.forEach(item => axios.get("https" + item.slice(4)).then(elem => console.log(elem)))).catch(() => alert("Houston, we have a problem while getting the Characters"));
}
...
function renderCharacters(param) {
  ...
  if (terminalID.childElementCount < 2) {
    for (let j = 0; j < 2; j++) {
      ...
      out.innerHTML = `<tr><td style="width: 76px;">Avatar</td>
  ...
  const out = document.createElement("table");
  outChars.innerHTML += `<tr><td style="width: 55px;">${urlAva}</td>
